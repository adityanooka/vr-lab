...
namespace VRLabClass.Milestone2
{
    public class AnchoredTeleportNavigation : MonoBehaviour
    {
        ...
        [SerializeField] private float _maxTeleportRadius = 5f; // Maximum distance from anchor point user can teleport (locked state only)
        ...
        private void EvaluateLockedInput()
        {
            ...
            // Constraining preview avatar position to max radius around anchor
            Vector3 directionFromAnchor = (_hitPoint - _anchorPosition).normalized;
            float distanceFromAnchor = Vector3.Distance(_hitPoint, _anchorPosition);
            
            if (distanceFromAnchor > _maxTeleportRadius)
            {
                // Clamp position to max radius
                _previewAvatarPosition = _anchorPosition + directionFromAnchor * _maxTeleportRadius;
            }
            else
            {
